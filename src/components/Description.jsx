...
const Description = ({ 
  ...
  let spanGenresEl = genres.map((item, index) => {
    if(index !== 2) {
      return <><span key = {index}>{item}</span><b>|</b></>
    } else {
    return  <><span key = {index}>{item}</span></>
    } })

    const allEpisodesBtn = () => {
      alert('You clicked Episode Button.')
    }

    const streamBtn = () => {
      alert('You clicked stream Button.')
    }
  return (
    <section>
      <div className="content">
        ...
        <div className="content_info">
          <span>
            <span className="content_info-white">2019</span>
          </span>
          <b>|</b>
          <span>
            <span className="content_info-white">DIRECTOR:</span>
            <span className="content_info-grey"> {director}</span>
          </span>
          <b>|</b>
          <span>
            <span className="content_info-white">seasons:</span>
            <span className="content_info-grey"> {seasons}</span>
          </span>
        </div>
        <p className="content_info-grey">
          {" "}
        ...
        <div>
          <span className="content-stars">
            <Stars />
          </span>
        </div>
        <div className="content_btns">
          <button className="content_btn-stream" onClick={streamBtn}>
            STREAM NOW{" "}
            <svg
              width="32"
              height="32"
          ...
          <button className="content_btn-episodes" onClick={allEpisodesBtn}>ALL EPISODES</button>
        </div>
      </div>
    </section>
  );
